...
SNET Publisher can host two separate components:

- **Hosted Service**: this FastAPI/Docker app, built from the repository root `Dockerfile`.
- **Daemon**: the SNET daemon that forwards marketplace calls to the hosted service.

The hosted service must answer liveness probes before model loading:

- `GET /`
- `GET /heartbeat`
- `GET /health`

The Daemon **Service Endpoint** must point to the SNET Hosted Service base URL generated by Publisher.
...
The hosted FastAPI app must expose the API routes that the daemon will call:

- `POST /api/session`
- `POST /api/layer`

Minimum Hosted Service requirements:

- Repository root `Dockerfile` builds successfully.
...
- HaaS with Hosted Service: SNET manages the hosted FastAPI container, daemon, SSL, and managed `etcd`
